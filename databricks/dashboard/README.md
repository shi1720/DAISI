...
Recorded on 22 September 2026 with **Databricks CLI 1.17.0**: all offline checks passed; the archive yielded 17 scheduled closed centres, six comparison rows, five source rows and 32 quarantine rows. These offline checks alone did not prove deployment. Subsequent cloud verification on the same date created and published dashboard `01f1b64b21b9194ea6f98fbdaa0437d2`, confirmed `embed_credentials=false`, and executed all four datasets successfully against the live publication. The server returned the expected `workspace` catalog and `hawkerbridge` schema on each dataset. Results contained one closure-count row, six comparison rows, five sources and 32 quarantined intervals. Visual rendering has not been independently verified. See [the published dashboard](https://dbc-6fea141d-04eb.cloud.databricks.com/dashboardsv3/01f1b64b21b9194ea6f98fbdaa0437d2/published?w=7474659582484876) and [dated deployment evidence](../../docs/databricks-verification.json).
