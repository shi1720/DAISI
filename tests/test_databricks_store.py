...
def response(rows=None, *, state="SUCCEEDED", names=None, **manifest):
    ...
    return {
        ...
        "result": result,
        "manifest": {
            "schema": {"columns": [{"name": name} for name in (names or [])]},
...
def setup_store():
    ...
    config = SimpleNamespace(
        databricks_warehouse_id="abc123", catalog="workspace", schema="hawkerbridge"
    )
...
def test_missing_config_and_identifier_injection_are_rejected():
    for warehouse, catalog, schema in [
        ("", "workspace", "hawkerbridge"),
        ("a", "workspace; DROP TABLE plans", "hawkerbridge"),
        ("a", "workspace", "bad.schema"),
    ]:
        with pytest.raises(ValueError):
            DatabricksStore(
                SimpleNamespace(databricks_warehouse_id=warehouse, catalog=catalog, schema=schema),
                client=Mock(),
            )
...
def test_pending_statement_is_polled_before_success(setup_store):
    ...
    api.get_statement.side_effect = [
        response(state="RUNNING"),
        response([["ok"]], names=["answer"]),
    ]
...
def test_chunks_are_fetched_via_sdk_and_nulls_preserved(setup_store):
    ...
    api.get_statement_result_chunk_n.assert_called_once_with(
        statement_id="statement-123", chunk_index=1
    )


@pytest.mark.parametrize(
    "problem", ["truncated", "missing_chunk", "loop", "bad_offset", "external_links"]
)
...
def test_owner_and_plan_payload_use_parameters_even_with_quotes(setup_store):
    ...
    assert (
        "target.owner_id = source.owner_id AND target.plan_id = source.plan_id" in args["statement"]
    )
...
def test_reads_and_deletes_remain_owner_scoped(setup_store):
    ...
    assert all(
        not c.kwargs["statement"].startswith("DELETE") for c in api.execute_statement.call_args_list
    )


def test_delete_returns_actual_affected_count_when_available(setup_store):
    store, api = setup_store
    api.execute_statement.side_effect = [
        response([[json.dumps({"id": "p"})]], names=["plan_json"]),
...
def test_conditional_update_uses_revision_and_never_inserts(setup_store):
    store, api = setup_store
    api.execute_statement.return_value = response([["1"]], names=["num_affected_rows"])
    plan = {"id": "plan-one", "updated_at": "2026-09-22T12:01:00Z", "notes": "new notes"}
    store.update_plan("owner", plan, "2026-09-22T12:00:00Z")
    args = api.execute_statement.call_args.kwargs
    assert args["statement"].startswith("UPDATE ")
    assert "INSERT" not in args["statement"] and "MERGE" not in args["statement"]
    assert "WHERE owner_id = :owner_id AND plan_id = :plan_id" in args["statement"]
    assert "get_json_object(plan_json, '$.updated_at') = :expected_updated_at" in args["statement"]
    assert {p.name: p.value for p in args["parameters"]}[
        "expected_updated_at"
    ] == "2026-09-22T12:00:00Z"


def test_delta_concurrent_transaction_is_a_revision_conflict_without_retry(setup_store):
    from hawkerbridge.plan_errors import PlanConflictError

    store, api = setup_store
    failed = response(state="FAILED")
    failed["status"]["error"] = {
        "error_code": "BAD_REQUEST",
        "message": "[DELTA_CONCURRENT_APPEND.ROW_LEVEL_CHANGES] Transaction conflict detected.",
    }
    api.execute_statement.return_value = failed
    with pytest.raises(PlanConflictError):
        store.update_plan("owner", {"id": "plan-one", "updated_at": "2026-09-22T12:01:00Z"}, "old")
    assert api.execute_statement.call_count == 1


def test_unrelated_sql_failure_is_not_reported_as_an_edit_conflict(setup_store):
    store, api = setup_store
    failed = response(state="FAILED")
    failed["status"]["error"] = {
        "error_code": "BAD_REQUEST",
...
@pytest.mark.parametrize("existing", [None, {"id": "plan-one", "updated_at": "newer"}])
def test_conditional_update_rejects_missing_and_stale_plan(setup_store, existing):
    from hawkerbridge.plan_errors import PlanConflictError, PlanNotFoundError

    store, api = setup_store
    api.execute_statement.side_effect = [
        response([["0"]], names=["num_affected_rows"]),
        response([[json.dumps(existing)]] if existing else [], names=["plan_json"]),
    ]
    with pytest.raises(PlanNotFoundError if existing is None else PlanConflictError):
        store.update_plan(
            "owner", {"id": "plan-one", "updated_at": "2026-09-22T12:01:00Z"}, "old-revision"
        )
    assert all(
        "INSERT" not in call.kwargs["statement"] for call in api.execute_statement.call_args_list
    )
...
def test_pipeline_publishes_last_and_records_real_evaluation(
    pipeline_module, fake_mlflow, monkeypatch
):
    from datetime import date, datetime
    from pathlib import Path
        ...
        def createDataFrame(self, rows, schema):
            for field in schema.split(", "):
                ...
                for row in rows:
                    ...
                    assert row[name] is None or isinstance(row[name], expected), (
                        name,
                        row[name],
                        expected,
                    )
    ...
    result = pipeline_module.run_pipeline(
        ...
        catalog="workspace",
        schema="hawkerbridge",
        app_name="hawkerbridge",
        input_mode="archived_replay",
        reuse_archived_geometry=True,
    ...
    assert all(
        row["weighted_benefit"] + 0.01 >= row["baseline_weighted_benefit"] for row in evaluations
    )
    code_hash = pipeline_module.engine_code_sha256()
    assert published["manifest"]["databricks"]["engine_code_sha256"] == code_hash
    assert all(
        json.loads(row["result_json"])["engine_code_sha256"] == code_hash for row in evaluations
    )
    assert any(
        call.args[0].get("engine_code_sha256") == code_hash
...
def test_pipeline_failure_does_not_publish_snapshot(pipeline_module, fake_mlflow, monkeypatch):
    ...
    written = []
    monkeypatch.setattr(
        pipeline_module, "bootstrap_tables", lambda *args: "`workspace`.`hawkerbridge`"
    )
    monkeypatch.setattr(pipeline_module, "grant_app_permissions", lambda *args: "principal-test")
    monkeypatch.setattr(
        pipeline_module, "append_rows", lambda spark, ns, table, rows: written.append((table, rows))
    )
    monkeypatch.setattr(
        pipeline_module, "evaluate", Mock(side_effect=ValueError("evaluation failed"))
    )
...
def test_cloud_evaluation_requires_matching_snapshot_and_complete_results(setup_store):
    ...
    assert (
        ...
        in api.execute_statement.call_args.kwargs["statement"]
    )
    row[2] = "different"
    api.execute_statement.return_value = response([row], names=names)
...
@pytest.mark.parametrize(
    "problem", ["missing_manifest_hash", "missing_result_hash", "mixed_code", "wrong_input"]
)
def test_cloud_evaluation_rejects_unversioned_or_mixed_implementations(setup_store, problem):
    ...
    rows = [
        [json.dumps(result), "fp", "fp", "pub1", "2026-09-22", "live", "2", expected]
...
def test_dashboard_cooked_food_density_excludes_zero_food_markets():
    ...
    with sqlite3.connect(":memory:") as database:
        ...
        database.execute(
            "CREATE TABLE published_demand_zones (planning_area TEXT, residents INTEGER, seniors INTEGER)"
        )
        ...
        database.executemany(
            "INSERT INTO published_demand_zones VALUES (?, ?, ?)",
