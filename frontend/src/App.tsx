...
export default function App() {
  ...
  const {mobile,sidebarRef} = useMobileNavigation(sidebar,closeSidebar);
  ...
  useEffect(() => {
    const expire = () => { setSession(current => current ? {...current,user:null,csrf_token:null} : null); setSnapshot(null); setAnalysis(null); setAnalysisKey(''); setOpenPlanId(null); setPage('overview'); setSidebar(false);setAccountOpen(false); };
  ...
  useEffect(() => {
    ...
    post<Analysis>('/analyse',params,controller.signal).then(result => { if (!controller.signal.aborted) { setAnalysis(result);setAnalysisKey(requestKey); } }).catch(e => { if (!controller.signal.aborted) setError(errorMessage(e)); }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  ...
  async function clearWorkspace() {
    if(session)useSessionToken({...session,user:null,csrf_token:null});setSession(null);
    setSnapshot(null);setAnalysis(null);setAnalysisKey('');setOpenPlanId(null);setPage('overview');setSidebar(false);setAccountOpen(false);await loadSession();
  }
  async function logout() {
    setLoggingOut(true);
    try { await post('/auth/logout',{});await clearWorkspace(); }
  ...
  const outdated = !!analysis && (analysisKey !== JSON.stringify(params) || loading || !!error);
  return <div className="app-layout"><a className="skip-link" href="#main">Skip to content</a>{sidebar && <button className="sidebar-scrim" aria-label="Close navigation" onClick={() => setSidebar(false)}/>}
    <aside ref={sidebarRef} id="workspace-navigation" inert={mobile && !sidebar ? true : undefined} aria-hidden={mobile && !sidebar ? true : undefined} role={mobile && sidebar ? 'dialog' : undefined} aria-modal={mobile && sidebar ? true : undefined} aria-label={mobile && sidebar ? 'Workspace navigation' : undefined} className={`sidebar ${sidebar ? 'is-open' : ''}`}><div className="sidebar-brand"><Brand/><button className="icon-button mobile-only" onClick={() => setSidebar(false)} aria-label="Close menu"><X size={20}/></button></div><div className="workspace-label"><span className="workspace-icon"><Command size={15}/></span><div>Singapore workspace<span>Community operations</span></div></div><div className="nav-caption">YOUR WORKSPACE</div><nav aria-label="Main navigation">{nav.map(item => <button key={item.id} className={`nav-item ${page === item.id ? 'active' : ''}`} onClick={() => navigate(item.id)} aria-current={page === item.id ? 'page' : undefined}><item.icon size={19}/><span>{item.label}</span>{page === item.id && <span className="nav-active-dot"/>}</button>)}</nav><div className="sidebar-bottom"><div className="sidebar-help"><div className="sidebar-help-icon"><CircleHelp size={19}/></div><h3>From insight to action.</h3><p>Every plan starts a conversation with a centre, a venue, and the people who use them.</p><button onClick={() => navigate('evidence')}>How the desk works <ArrowUpRight size={15}/></button></div><div className="account"><span className="avatar">{session.user.name.slice(0,1).toUpperCase()}</span><div><strong>{session.user.mode === 'guest' ? 'Guest workspace' : session.user.name}</strong><span>{session.user.mode === 'guest' ? 'Private to this session' : session.user.mode === 'databricks' ? 'Databricks identity' : 'Personal workspace'}</span></div>{session.user.mode !== 'databricks' && <button className="icon-button" onClick={() => void logout()} disabled={loggingOut} aria-label="Sign out"><LogOut size={17}/></button>}</div></div><div className="sidebar-foot">HAWKERBRIDGE <span>MADE FOR SINGAPORE</span></div></aside>
    <div className="main-shell" inert={mobile && sidebar ? true : undefined}><header className="topbar"><div className="breadcrumb"><button className="icon-button mobile-only" aria-label="Open navigation" aria-expanded={sidebar} aria-controls="workspace-navigation" onClick={() => setSidebar(true)}><Menu size={22}/></button><span className="breadcrumb-home">Workspace</span><span className="breadcrumb-slash">/</span><strong>{nav.find(x => x.id === page)?.label}</strong></div><div className="topbar-right"><span className="public-data-badge"><span className="status-dot"/> Public data snapshot{snapshot?.manifest.closures_year ? ` · ${snapshot.manifest.closures_year}` : ""}</span>{session.user.mode === 'databricks' ? <div className="top-avatar" aria-label={session.user.name}>{session.user.name.slice(0,1).toUpperCase()}</div> : <button className="top-avatar" aria-label="Account settings" title="Account settings" onClick={() => setAccountOpen(true)}>{session.user.name.slice(0,1).toUpperCase()}</button>}</div></header>
    <main id="main" tabIndex={-1} className="main-content"><div className="utility-bar"><span className="edition-label">SINGAPORE <span>01°17′N 103°51′E</span></span>{(page === 'overview' || page === 'planner') && <div className="global-filters"><label className="radius-control area-control"><span className="sr-only">Planning area scope</span><select aria-label="Planning area scope" value={params.planning_areas[0] ?? ""} onChange={e => setParams(p => ({...p,planning_areas:e.target.value ? [e.target.value] : [],rescheduled_closure_ids:[]}))}><option value="">All Singapore</option>{planningAreas.map(area => <option key={area} value={area}>{area}</option>)}</select><ChevronDown size={14}/></label><label className="date-control"><CalendarDays size={16}/><span className="sr-only">Analysis date</span><input type="date" value={params.date} min={dateMin} max={dateMax} onChange={e => { if(e.target.value && e.target.validity.valid) setParams(p => ({...p,date:e.target.value,rescheduled_closure_ids:[]})); }}/></label><label className="radius-control"><span className="sr-only">Nearby access threshold</span><select aria-label="Nearby access threshold" value={params.radius_m} onChange={e => setParams(p => ({...p,radius_m:Number(e.target.value)}))}><option value={400}>400 m access</option><option value={600}>600 m access</option><option value={800}>800 m access</option><option value={1000}>1 km access</option><option value={1200}>1.2 km access</option><option value={1500}>1.5 km access</option></select><ChevronDown size={14}/></label></div>}</div>
    ...
    {analysis && snapshot && <div hidden={page !== 'overview' && page !== 'planner'} className={`analysis-content ${outdated ? 'is-updating' : ''}`} aria-busy={loading}>{loading && <div className="updating-pill"><Spinner small label="Updating access outlook…"/></div>}{!loading && error && <div className="notice warning" role="status">The last successful outlook is shown below. It may not match your current inputs. Retry the refresh before generating a proposal.</div>}{page === 'overview' && <Overview analysis={analysis} snapshot={snapshot} onPlan={() => navigate('planner')} onDate={value => setParams(p => ({...p,date:value,rescheduled_closure_ids:[]}))} onEvidence={() => navigate('evidence')}/>}<div hidden={page !== 'planner'}><Planner analysis={analysis} snapshot={snapshot} params={params} setParams={setParams} analysisLoading={outdated} onSaved={id => { setPlansVersion(x => x+1); setOpenPlanId(id); navigate('plans'); notify('Plan saved to your workspace.'); }}/></div></div>}
    ...
    <footer className="workspace-footer"><span>HawkerBridge <i/> Keeping neighbourhoods at the table.</span><span>{snapshot?.manifest.fetched_at ? `Snapshot ${day(snapshot.manifest.fetched_at)}` : 'Singapore open data'} · Planning support, not a service guarantee</span></footer></main></div>{accountOpen && <AccountSettings session={session} onClose={() => setAccountOpen(false)} onDeleted={clearWorkspace} onLogout={logout}/>} {toast && <Toast>{toast}</Toast>}
  </div>;
}
